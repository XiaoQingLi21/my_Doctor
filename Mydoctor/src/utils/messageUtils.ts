...
// Cache to store token counts of messages
...
export const countTokens = (messages: MessageInterface[], model: ModelOptions) => {
  ...
  messages.forEach((message, index) => {
    const cacheKey = `${index}-${message.content}`;
    if (tokenCountCache.has(cacheKey)) {
      // Use cached count
      totalTokenCount += tokenCountCache.get(cacheKey);
    } else {
      // Count tokens and update cache
      const count = getChatGPTEncoding([message], model).length;
