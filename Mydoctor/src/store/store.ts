...
interface InitSlice {
  ...
  setChatsInitialized: (initialized: boolean) => void;
}

const createInitSlice: StoreSlice<InitSlice> = (set) => ({
  chatsInitialized: false,
  setChatsInitialized: (initialized) => set({ chatsInitialized: initialized }),
});
...
export type StoreState = ChatSlice &
  ...
  ToastSlice &
  UserSlice &
  InitSlice; // Add InitSlice to your StoreState
...
const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      ...createInitSlice(set, get), // Add the init slice here
    }),
    {
      name: 'free-chat-gpt',
